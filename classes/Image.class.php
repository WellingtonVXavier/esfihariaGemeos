...
####################################################################################################################
#Image.class.php
#Biblioteca com m�todos para redimencionar imagens
#
...
class Image
{
	private $pointer = 0, $check; //Gera os ponteiros para trabalhar com v�rias imagens
	private $image, $width, $height, $mime, $size; //Atributos para informa��es da imagem de entrada
	...
	private function properties($pointer)
	{
		...
		if ($this->re_resize[$pointer] == self::$_IMG_RESIZE_FIXED) {
			return array(
				'resized_width' => $this->re_width[$pointer],
			...
			return array(
				'resized_width' => $array[0],
				'resized_height' => $array[1],
				'width' => $array[2],
				'height' => $array[3]
			);
		}
